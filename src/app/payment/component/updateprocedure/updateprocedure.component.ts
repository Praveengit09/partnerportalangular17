...
@Component({
  selector: 'updateprocedure',
  templateUrl: './updateprocedure.template.html',
  styleUrls: ['./updateprocedure.style.scss'],
...
export class UpdateProcedureComponent implements OnInit {
  ...
  onUploadButtonClick() {
    ...
    this.fileUtil.fileUploadToAwsS3(null, this.uploadFilesList[0], 0, false, false).then((awsS3FileResult) => {
      ...
      this.paymentService.readProcedureExcel({ fileName: awsS3FileResult.Location, pocId: this.authService.selectedPOCMapping.pocId }).then((data) => {
        this.spinnerService.stop();
