...
@Component({
  selector: 'updateadmissionnote',
  templateUrl: './updateadmissionnote.template.html',
  styleUrls: ['./updateadmissionnote.style.scss'],
  encapsulation: ViewEncapsulation.Emulated
})
export class UpdateAdmissionNoteComponent implements OnInit {
  config: any;
  month: any;
  year: any;
  startdate: Date;
  ...
  onUploadButtonClick() {
    ...
    this.fileUtil.fileUploadToAwsS3(null, this.uploadFilesList[0], 0, false, false).then((awsS3FileResult) => {
      ...
      this.paymentService.readAdmissionNoteExcel({ fileName: awsS3FileResult.Location, pocId: this.authService.selectedPocDetails.pocId }).then((data) => {
        this.spinnerService.stop();
