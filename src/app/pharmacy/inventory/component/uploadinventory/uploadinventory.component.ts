...
export class UploadInventoryComponent implements OnInit {
  ...
  onUploadButtonClick() {
    ...
    this.fileUtil.fileUploadToAwsS3(null, this.uploadFilesList[0], 0, false, false).then((awsS3FileResult) => {
      ...
      this.pharmacyService.readInventoryExcel({ fileName: awsS3FileResult.Location, pocId: this.authService.selectedPocDetails.pocId, empId:this.authService.userAuth.employeeId }).then((data) => {
        this.spinnerService.stop();
