...
@Component({
    selector: 'uploaddeliverycharges',
    templateUrl: './uploaddeliverycharges.template.html',
    styleUrls: ['./uploaddeliverycharges.style.scss'],
    encapsulation: ViewEncapsulation.Emulated
})
export class UploadDeliveryChargesComponent implements OnInit {
    config: any;
    month: any;
    year: any;
    startdate: Date;
    ...
    onUploadButtonClick() {
        ...
        this.fileUtil.fileUploadToAwsS3(null, this.uploadFilesList[0], 0, false, false).then((awsS3FileResult) => {
            ...
            this.pharmacyService.readDeliveryChargesExcel({ fileName: awsS3FileResult.Location, pocId: this.authService.selectedPocDetails.pocId, empId: this.authService.userAuth.employeeId }).then((data) => {
                this.spinnerService.stop();
                if (data.statusCode == 405) {
                    ...
                    this.errorMessage = new Array();
                    this.showMessage = true;
                    this.errorMessage[0] = "All Delivery Charges updated successfully.";
                    var $el = $('#files');
