...
@Component({
  selector: '[ordersummary-homeorder-component]',
...
export class NewOrderSummaryComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.cartItem = this.pharmacyService.cartItem;
    if (this.cartItem) {
  ...
  ConfirmOrder() {
    ...
    basketRequest = { ...this.pharmacyService.pharmacyDeliveryDetails, ...basketRequest }
    this.diagnosticService.initiatePayment(basketRequest).then(reportResponse => {
      ...
      if (reportResponse.statusCode == 201 || reportResponse.statusCode == 200) {
        ...
        this.pharmacyService.cartItem = { ...reportResponse.cartItemList[0] };
  ...
  gotoProductInvoice() {
    ...
    if (this.paymentModeIndex == Payment.PAYMENT_TYPE_MOBILE) this.router.navigate(['/app/pharmacy/homeorder']);
    else this.router.navigate(['/app/pharmacy/homeorder/new-invoice']);
  }
