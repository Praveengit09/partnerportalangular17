...
export class PharmacyHomeOrderComponent implements OnInit, OnDestroy {
  config: any;
  perPage: number = 10;
  total: number = 0;
  searchCriteria: string = 'orderId';
  pharmacyHomeListIndex: string = 'pharmacyHomeListIndex';
  ...
  columns: any[] = [
    ...
    },
    {
      ...
      variable: 'cartItem.patientProfileDetails.title cartItem.patientProfileDetails.fName cartItem.patientProfileDetails.lName, \n cartItem.patientProfileDetails.age  cartItem.patientProfileDetails.gender, \n cartItem.patientProfileDetails.contactInfo.mobile ',
      filler: ',',
      filter: 'nametitle',
      sort: false
    },
    ...
    {
      display: 'Price',
      variable: 'cartItem.payment.amountPaid',
      filter: 'number',
      sort: false,
      conditions: [{
        value: 0,
  ...
  ngOnInit(): void {
    ...
    //To Recognise  Edited orders or not
    this.pharmacyService.pharmacyDeliveryDetails = undefined;
    this.pharmacyService.isEditedOrder = true;
    this.pharmacyService.isHomeOrder = true;
    this.pharmacyService.tempPharmacyDeliveryDetails = undefined;
  ...
  searchPharmacyDeliveryList() {
    ...
    this.pharmacyService.getpharmacydeliveries(this.adminPharmacyDeliveryRequest).then(deliveryList => {
      ...
      if (this.pharmacyDeliveryResponseList.length == 0) {
        ...
        if (this.pharmacyDeliveryResponseList != null && this.pharmacyDeliveryResponseList != undefined) {
          this.pharmacyDeliveryResponseList.forEach(element1 => {
            ...
            if (element1.cartItem && element1.cartItem.payment) {
              let usedWalletAmount = element1.cartItem.payment.usedWalletAmount;
              element1.orderPrice = usedWalletAmount ? +element1.cartItem.payment.finalAmount + +usedWalletAmount : +element1.cartItem.payment.finalAmount;
            }
      ...
      this.dataMsg = "Error occurred while processing request. Please try again!";
    });
  }
  ...
  newOrder() {
    this.router.navigate(['/app/pharmacy/homeorder/new-order']);
  }
