...
@Component({
    selector: 'homeadminorderdetails',
...
export class CentralHomeOrderDetailsComponent implements OnInit, OnDestroy {
    ...
    onGenerateBack(): void {
        this.router.navigate(['/app/pharmacy/centralhomeorder/listing']);
    }
    ...
    updatepharmacydeliveries(): void {
        ...
        this.pharmacyService.updatepharmacydeliveries(this.processOrderDetails).then(baseResponse => {
            ...
            if (baseResponse.statusCode == 201) {
                this.isError = false;
                this.errorMessage = new Array();
                if (this.processOrderDetails.actionStatus == this.actionStatus.REJECTED) {
    ...
    getPocList(empId: number): void {
        this.businessAdminService.getPOCForEmployeeByLocationMapping(empId, true).then(response => {
