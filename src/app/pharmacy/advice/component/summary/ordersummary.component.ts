...
export class NewOrderSummaryComponent implements OnInit {
  ...
  specialistList = [];
  enableSpecializationSelection : boolean = false;
  ...
  ngOnInit() {
    ...
    if (Config.portal && Config.portal.pharmacyOptions && Config.portal.pharmacyOptions.enablePharmacySpecializationSelection){ //MYMEDIC feature
        this.enableSpecializationSelection =true;
        this.pharmacyService.getSpeciality(0).then(response => {
  ...
  setServiceDetails(serviceIndex : number){
    if(serviceIndex >= 0){
      this.cartItem.serviceId = this.specialistList[serviceIndex].id;
      this.cartItem.serviceName = this.specialistList[serviceIndex].text;
    }
  }
  ...
  onEditOrderClickHandler() {
    ...
    this.pharmacyService.pharmacyAdviseTrack = this.cartItem;
    this.router.navigate(['/app/pharmacy/advice/newadvice']);
    // if (this.cartItem.inPatientBilling == true && this.cartItem.billNo)
  ...
  async gotoPharmacyInvoice() {
    ...
    // this.pharmacyService.pharmacyAdviseTrack.billNoList && this.pharmacyService.pharmacyAdviseTrack.billNoList.length > 0 ? this.router.navigate(['/app/pharmacy/inpatientorders/invoice']) : this.router.navigate(['/app/pharmacy/advice/invoice']);
