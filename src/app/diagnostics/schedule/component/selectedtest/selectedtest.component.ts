...
export class SelectedTestComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.getDiagnosticListBasedOnFirstChar();
    ...
    getDiagnosticListBasedOnFirstChar() {
        console.log("scheduleId, PocId & FirstChar---->" + this.scheduleId + " & " + this.pocId + " & " + this.firstChar + " & " + this.scheduleType);
        this.diagSchService.getDiagnosticListBasedOnFirstChar(this.scheduleId, this.pocId, this.firstChar, this.scheduleType).then(respList => {
    ...
    onOthersClick() {
        ...
        this.col2Checked = false;
        this.col3Checked = false;
        this.diagSchService.getDiagnosticListForOtherChar(this.pocId).then(respList => {
            console.log("response in getDiagnosticListBasedOnFirstChar:: " + JSON.stringify(respList));
            this.divisionOfTheList(respList);
            if (this.selectedInvestigationDetailsList.length > 0) {
                setTimeout(() => {
    ...
    saveAndContinue() {
        ...
        this.router.navigate(['/app/diagnostics/schedule/savedtest/']);
    }
