...
export class EditReturnComponent implements OnInit {
    ...
    remove(index: number): void {
        ...
        this.processOrderDetails.cartItem.productList.splice(index, 1);
    ...
    onQuantityChange(): void {
        ...
        if (this.processOrderDetails.cartItem.productList != null && this.processOrderDetails.cartItem.productList.length > 0) {
            this.processOrderDetails.cartItem.productList.forEach(element => {
                if (element == undefined || element == null) {
                    ...
                    if (element.productId == undefined || element.productId <= 0) {
                        this.isError = true;
                        this.errorMessage = new Array();
                        this.errorMessage[0] = "Please add product to proceed further";
    ...
    onSubmitForCalculation() {
        ...
        if (!this.processOrderDetails.cartItem.productList || this.processOrderDetails.cartItem.productList.length <= 0) {
            this.errorMessage[0] = "Please add product to proceed.";
            this.isError = true;
            this.showMessage = true;
            return;
        }
        ...
        this.adminService.calculateProductDeliveries(this.processOrderDetails.cartItem).then(cartItemResponse => {
            ...
            if (cartItemResponse.statusCode == 200 || cartItemResponse.statusCode == 201) {
                ...
                this.router.navigate(['/app/product/returns/summary']);
            } else {
                this.isError = true;
                this.errorMessage = new Array();
