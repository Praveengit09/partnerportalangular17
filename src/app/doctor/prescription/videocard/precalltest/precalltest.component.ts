...
export class PreCallTestComponent implements OnInit,OnDestroy {
  ...
  getTestVideoSession(){
    this.doctorService.generateTestVideoSessionid().then((data)=>{
  ...
  skipTest(){
    this.videoCardService.updateSessionStatus('Doctor Skiped Network Test',2);
    this.doctorService.isNetworkQualityTested=true;
    this.navigateTo(this.surl,1);
  }

  initiateTest() {
    this.videoCardService.updateSessionStatus('Doctor Network Test Started',3);
    ...
    // this.API_KEY = this.doctorService.API_KEY;
    // this.SESSION_ID = this.doctorService.SESSION_ID;
    // this.TOKEN = this.doctorService.TOKEN;
  ...
  testStreamingCapability(subscriber, callback, self) {
    self.performQualityTest({ subscriber: subscriber, timeout: self.TEST_TIMEOUT_MS }, function (error, results) {
      ...
      self.performQualityTest({ subscriber: subscriber, timeout: 5000 }, function (error, results) {
        ...
        self.doctorService.audioSupported = results.audio.bitsPerSecond > 25000 &&
          results.audio.packetLossRatioPerSecond < 0.05;
  ...
  onTestComplete(self) {
    ...
    self.videoCardService.updateSessionStatus('Doctor Network Test Completed',5,quality);
    setTimeout(()=>{
